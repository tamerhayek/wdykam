		...
		{#each question.answers as { text: answer, image, participants, isCorrect }, index}
			{#if answer}
				<div
					...
					class="flex flex-col justify-between gap-3 overflow-hidden rounded-xl p-3 shadow-xl"
					class:bg-error={!isCorrect}
					class:bg-success={isCorrect}
				>
					<div class="flex items-center gap-2">
						{#if image}
							<figure transition:scale class="shrink-0 p-3">
								<img
					...
					{#if participants.length > 0}
						<div transition:slide class="flex flex-col gap-3 px-3">
							<div class="avatar-group -space-x-6 rtl:space-x-reverse">
								{#each participants as { id, name, avatar } (id)}
									<div animate:flip class="avatar border">
										<div class="w-12">
											<img
